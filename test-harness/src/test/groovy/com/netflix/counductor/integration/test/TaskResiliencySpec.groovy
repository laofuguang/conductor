...
//    @Inject
    ...
    def "verify mocks" () {
        ...
        then: "check mock counts"
//        1 * executionDAOFacade.createWorkflow
    }
    ...
    def "Verify that a workflow recovers and completes on schedule task failure from queue push failure"() {
        ...
        dynamicFailureProbability.setClassName("*")
    ...
    def "Verify that a workflow recovers and completes on schedule task failure from persistence failure"() {
        ...
        when: "The first task 'integration_task_1' is polled and completed"
        dynamicFailureProbability.setClassName("DynoQueueDAO")
        dynamicFailureProbability.setMethodName("push")
        dynamicFailureProbability.set(1)
        def task1Try1 = workflowTestUtil.pollAndCompleteTask('integration_task_1', 'task1.integration.worker')

        then: "Verify that the task was polled and acknowledged"
        verifyPolledAndAcknowledgedTask(task1Try1)

        and: "Ensure that the next task is not SCHEDULED"
        with(workflowExecutionService.getExecutionStatus(workflowInstanceId, true)) {
            status == Workflow.WorkflowStatus.RUNNING
            tasks.size() == 1
            tasks[0].taskType == 'integration_task_1'
        ...
        // Next steps:
        // Run a decide on the workflow
        // Ensure next task is scheduled
        // Ensure workflows resumes successfully

        when: "Running a decide on the workflow (supposedly from sweeper service)"
        dynamicFailureProbability.set(0)

        workflowExecutor.decide(workflowInstanceId)

        then: "verify that the next task is now scheduled successfully"
        with(workflowExecutionService.getExecutionStatus(workflowInstanceId, true)) {
            status == Workflow.WorkflowStatus.RUNNING
            tasks.size() == 2
            tasks[1].taskType == 'integration_task_2'
